...
def _apply_width_floor(family: str, params: dict, min_scale: float) -> dict:
    ...
    if family == "normal":
        ...
        # The anti-spike requirement is nominal: no component narrower than
        # ``min_scale``, which for a lognormal means sigma >= min_scale / median.
        # MIN_LOG_SIGMA is only the fallback for a median that cannot supply that
        # ratio. It used to be combined with max(), which silently widened every
        # question whose range is small next to its level -- on 45412 (range $2T
        # around a $40.7T level) it pushed sigma 0.0042 -> 0.03, a 7x widening
        # that pushed 41% of the run's mass off the grid and got the run dropped.
        floor = MIN_LOG_SIGMA
        if median > 0 and np.isfinite(median):
            floor = min(min_scale / median, 0.5)
