# Legacy Binary Prompt (tool-era)

**Name in code:** `BINARY_PROMPT_TEMPLATE` (docs/templatePrompts.py)  
**Source:** [docs/templatePrompts.py:1](../../docs/templatePrompts.py#L1)  
**Tier:** n/a — never called

> **Not in the pipeline.** A standalone copy in `docs/`, imported by nothing. Superseded by [binary-prompt-template](../01-active/binary-prompt-template.md).

## Intended use

**Pipeline stage:** None.

The pre-overhaul binary prompt. Historically interesting because it is built around a `run_python_code` tool — '**ALWAYS use this tool for any math … never do mental arithmetic**' — and each phase calls for a Python tool call. That tool still exists in `llm_client.py` but `use_tools=True` appears nowhere in the codebase, so the live prompt asks for the same arithmetic with no calculator attached.
...
Same slots as the live binary prompt, minus the later additions.

## Length

- **Scaffold (this file's template text):** 4,902 chars, 6 slot(s)
- **Filled prompt as sent:** ~4.9K chars scaffold — about 3.5× shorter than its live replacement
