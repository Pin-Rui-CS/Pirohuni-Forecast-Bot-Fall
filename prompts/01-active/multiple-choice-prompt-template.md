...
**Name in code:** `MULTIPLE_CHOICE_PROMPT_TEMPLATE` (forecasters/multiple_choice.py)  
**Source:** [forecasters/multiple_choice.py:18](../../forecasters/multiple_choice.py#L18)  
...
**Call site:** `gather_forecast_runs(..., "mc-forecast")`, temperature 0.3, 3 runs

## Intended use

**Pipeline stage:** Forecasting — multiple-choice questions.

The binary prompt's structure adapted to a fixed option set: same Phase 0 → 4 sequence, but every phase outputs a full distribution that must sum to 100%. Adds a cumulative/monotonic warning (leaderboards and running totals are not races, so leader persistence is high) and a residual-bucket floor for re-categorisation risk.
...
`{options}` appears twice — once as context, once to fix the output order. Same brief-vs-raw split as the binary prompt.

## Length

- **Scaffold (this file's template text):** 13,566 chars, 7 slot(s)
- **Filled prompt as sent:** ~23K – 60K chars; up to ~215K on the heterogeneous raw run

## Notes

Lacks the binary prompt's EVENT CHAIN block and its Directional-audit / Discount-ledger final lines — the entailment rules appear only in condensed form inside Phase 1.
