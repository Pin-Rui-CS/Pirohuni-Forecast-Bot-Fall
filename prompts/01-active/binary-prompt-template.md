...
**Name in code:** `BINARY_PROMPT_TEMPLATE` (forecasters/binary.py)  
**Source:** [forecasters/binary.py:22](../../forecasters/binary.py#L22)  
...
**Call site:** `gather_forecast_runs(..., "binary-forecast")`, temperature 0.3, 3 runs

## Intended use

**Pipeline stage:** Forecasting — binary questions.

The main forecasting prompt for yes/no questions. Runs a fixed phase sequence: Phase 0 research audit (with a temporal-validity check), Phase 0.5 resolution mechanics with the EVENT CHAIN / floor-vs-ceiling entailment rules, Phase 1 outside view, Phase 2 inside view with the timing-signal arithmetic, Phase 3 adversarial synthesis, Phase 4 pre-mortem, then a final block with a directional audit and a discount ledger. Ends with `"Probability: ZZ%"`. Much of the phase-specific text is scar tissue from named post-mortems — the floor/ceiling and on-schedule rules trace to the 44620 miss.
...
`{summary_report}` is the compiled brief for runs 1–2, and the RAW deduplicated research (capped at `_RAW_VIEW_MAX_CHARS = 200,000`) for the Tier 2 heterogeneous run. `{today}` appears three times and drives the temporal-validity check.

## Length

- **Scaffold (this file's template text):** 17,249 chars, 6 slot(s)
- **Filled prompt as sent:** ~27K – 65K chars on the compiled brief; up to ~220K chars on the heterogeneous raw run

## Notes

**No tools.** `gather_forecast_runs` hard-codes `use_tools=False`, so every calculation this prompt demands is done mentally. The `run_python_code` tool still exists in `llm_client.py` but is unreachable — see [legacy-binary-prompt-template](../03-inactive/legacy-binary-prompt-template.md), which was written when the tool was live.
