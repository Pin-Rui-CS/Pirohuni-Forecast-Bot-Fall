...
**Name in code:** `_build_ranking_prompt` (research/serp_research.py)  
**Source:** [research/serp_research.py:984](../../research/serp_research.py#L984)  
**Tier:** Tier 2 — `anthropic/claude-sonnet-5` (OpenRouter) / `gpt-5.6-terra` (OpenAI)
  
**Call site:** `DEFAULT_SERP_RANKING_MODEL`, temperature 0.2

## Intended use

**Pipeline stage:** Research step 4 — picks which search results get scraped.

Chooses up to `DEFAULT_MAX_RANKED_URLS = 20` URLs from up to `_MAX_RANKING_INPUT_RESULTS = 80` candidates and groups them by research purpose. The group labels and per-URL 'purpose' strings it invents are reused by the extract step and the retry logic, so this prompt sets the vocabulary for the rest of the research run.

**Related prompts:** Near-duplicate of [tavily-ranking-prompt](../02-conditional/tavily-ranking-prompt.md) and [firecrawl-ranking-prompt](../02-conditional/firecrawl-ranking-prompt.md). The three have drifted: this one and Tavily's are ~2.5K chars, Firecrawl's is ~4.1K and carries an extra date-discipline paragraph plus different candidate fields.
...
Each candidate renders as Title / URL / Date / Query / Snippet.

## Length

- **Scaffold (this file's template text):** 2,532 chars, 8 slot(s)
- **Filled prompt as sent:** ~5K – 25K chars (80 candidates × ~250 chars)
